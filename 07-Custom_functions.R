...
load_3pREADS_BAM_file <- function(bamfile, filter_pass = TRUE) {
  message("Loading 3'READS BAM file ", BiocGenerics::basename(bamfile), "...")
  ga <- readGAlignments(bamfile, use.names = TRUE, param = ScanBamParam(what = "seq")) %>% sortSeqlevels()
  if (isTRUE(filter_pass)) {
    # Detect PASS (PAS supporting) reads with at least two T's immediately preceding the 5' aligned part:
    message("\t", length(ga), " input reads;"); flush.console()
    gr <- pmapToAlignments(granges(ga), ga) %>% unname()
    seq <- ga %>% as.data.frame() %>% .$seq %>% as("DNAStringSet")
    up <- subseq(seq, start = 1, end = start(gr) - 1)
    down <- subseq(seq, start = end(gr) + 1, end = width(seq))
    lgl_1 <- grepl("TT$", up)
    lgl_2 <- grepl("^AA", down)
    pass <- ifelse(strand(ga) == "+", lgl_1, lgl_2)
    message("\t", sum(pass), " PASS reads (", round(mean(pass) * 100), "%);")
    ga <- ga[pass]
  }
  gr <- ga %>% granges() %>% resize(1, "start")
  strand(gr) <- ifelse(strand(gr) == "+", "-", "+")
  out <- TranscriptomeReconstructoR:::convert_GRanges_to_coverage(gr)
  return(out)
}
